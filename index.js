...
async function cmdChannel(interaction) {
  ...
  await channel.send({content: 'You can click the button below or use `/ticket open` to open a ticket! A private thread will be created where you can communicate with server staff.', components: [row], withResponse: true})
}
